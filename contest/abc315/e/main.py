...
def main():
    ...
    for i in range(N):
        ...
        read_books.append(P if P else [])
    toread = []
    is_read = set()

    def read(x, is_read, to_read):
        if x in is_read:
            return
        need_read = set(read_books[x])-is_read
        for y in need_read:
            read(y, is_read, to_read)
        to_read.append(x)
        is_read.add(x)

    read(0, is_read, toread)
    print(*map(lambda x: x+1, toread[:-1]))
