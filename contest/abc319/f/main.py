        ...
        for to in edge[v]:
            ...
            if t == 1:
                if s <= power:
                    ...
                    heapq.heappush(next_enemy, [s, to, g])
            else:
                heapq.heappush(portion, g)
                q.append(to)
        if not q and next_enemy:
            s, to, g = heapq.heappop(next_enemy)
            while power < s:
                if not portion:
                    flg = False
                    return
                if power*max(portion) < s:
                    power *= heapq.heappop(portion)
                else:
                    portion.sort()
                    for p in portion:
                        if power*p >= s:
