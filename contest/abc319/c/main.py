    ...
    for chain in itertools.permutations(range(9)):
        ...
        for i in chain:
            ...
            if c+r == 2:
                cnt[7] += 1
            cnt_num[c].add(C[r][c])
            cnt_num[3+r].add(C[r][c])
            if c == r:
                cnt_num[6].add(C[r][c])
            if c+r == 2:
                cnt_num[7].add(C[r][c])
            if cnt[c] >= 2 and len(cnt_num[c]) == 1:
                flg = False
                break
            if cnt[3+r] >= 2 and len(cnt_num[3+r]) == 1:
                flg = False
                break
            if c == r and cnt[6] >= 2 and len(cnt_num[6]) == 1:
                flg = False
                break
            if c+r == 2 and cnt[7] >= 2 and len(cnt_num[7]) == 1:
                flg = False
                break
        if flg:
            ans += 1
