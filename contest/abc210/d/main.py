...
def main():
    H, W, C = MI()
    A = LII(H)
    dp1 = [[float('inf')] * W for _ in range(H)]
    dp2 = [[float('inf')] * W for _ in range(H)]
    B = [a[::-1] for a in A]
    dp1[0][0] = A[0][0]
    dp2[0][0] = B[0][0]
    ans = float('inf')
    for i in range(H):
        for j in range(W):
            if i > 0:
                dp1[i][j] = min(dp1[i-1][j]+C, dp1[i][j])
                dp2[i][j] = min(dp2[i-1][j]+C, dp2[i][j])
            if j > 0:
                dp1[i][j] = min(dp1[i][j-1]+C, dp1[i][j])
                dp2[i][j] = min(dp2[i][j-1]+C, dp2[i][j])
            if i > 0 or j > 0:
                ans = min(ans, dp1[i][j]+A[i][j])
                ans = min(ans, dp2[i][j]+B[i][j])
            # print(dp)
            dp1[i][j] = min(dp1[i][j], A[i][j])
            dp2[i][j] = min(dp2[i][j], B[i][j])
    print(ans)
