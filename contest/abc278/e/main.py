...
H, W, N, h, w = MI()
A = LII(H)
appearance = [[H+1, -1, W+1, -1] for _ in range(N+1)]
for i in range(H):
    for j in range(W):
        x = A[i][j]
        appearance[x][0] = min(i, appearance[x][0])
        appearance[x][1] = max(i, appearance[x][1])
        appearance[x][2] = min(j, appearance[x][2])
        appearance[x][3] = max(j, appearance[x][3])
ans = [[N]*(W-w+1) for _ in range(H-h+1)]
for i in range(H-h+1):
    for j in range(W-w+1):
        for x in range(1, N+1):
            if i <= appearance[x][0] and appearance[x][1] < i+h and j <= appearance[x][2] and appearance[x][3] < j+w:
                ans[i][j] -= 1
for a in ans:
    print(*a)
