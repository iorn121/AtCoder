...
# 切り上げ
def ceiling(x):
    Decimal(x).quantize(Decimal('0.1'), rounding=decimal.ROUND_CEILING)


# 切り捨て
def floor(x):
    Decimal(x).quantize(Decimal('0.1'), rounding=decimal.ROUND_FLOOR)
...
# ans = 0
# while True:
#     a, b, cnt = heapq.heappop(candidate)
#     a = -a
#     b = -b
#     if a == A:
#         ans = cnt
...
#     while True:
#         if a+k_divisors[-1] <= A:
#             print(a, b, k_divisors[-1])
#             heapq.heappush(candidate, (-a-k_divisors[-1], -b-k_divisors[-1], cnt+1))
#             break
#         else:
#             k_divisors.pop()
# print(ans)
...
# ans = 0
# pre_div = [1]
# for div in k_divisors:
#     if div < l:
#         pre_div.append(div)
#         continue
#     sa = B % div
#     i = -1
#     while sa % pre_div[i] != 0:
#         i -= 1
#     ans += sa//pre_div[i]
#     A -= sa
#     B -= sa
