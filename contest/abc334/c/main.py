...
def main():
    ...
    A=LI()
    if K%2==0:
        sa=[abs(A[2*i+1]-A[2*i]) for i in range(K//2)]
        print(sum(sa))
    elif K==1:
        print(0)
    else:
        sa1=[abs(A[2*i+1]-A[2*i]) for i in range(K//2)]
        sa2=[abs(A[2*i+2]-A[2*i+1]) for i in range(K//2)]
        # print(sa1,sa2)
        tmp1=0
        tmp2=sum(sa2)
        k1=0
        k2=0
        ans=min(tmp1+tmp2,sum(sa1))
        for i in range(1,K-1):
            if i%2==0:
                ...
                tmp2-=sa2[k2]
                k2+=1
                tmp=tmp1+tmp2+abs(A[i-1]-A[i+1])
            # print(i,tmp1,tmp2,tmp)
            ans=min(ans,tmp)
        print(ans)
