            ...
            if field[i][j]=="#":
                ...
                while que:
                    ...
                    for dh,dw in [[1,0],[-1,0],[0,1],[0,-1]]:
                        nh=h+dh
                        nw=w+dw
                        if 0<=nh<H and 0<=nw<W and field[nh][nw]=="#" and label[nh][nw]==-1:
                            label[nh][nw]=k
                            que.append(nh*W+nw)
                k+=1
            else:
    ...
    for r in red:
        ...
        for dh,dw in [[1,0],[-1,0],[0,1],[0,-1]]:
            ...
            if 0<=nh<H and 0<=nw<W and field[nh][nw]=="#":
                seen.add(label[nh][nw])
        if len(seen)==0:
            cnt+=1+k
        else:
            cnt+=k-len(seen)+1
    # print(cnt)
    print(cnt*mod_hw%MOD)
