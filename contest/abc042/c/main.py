...
N, K = MI()
D = set(LS())
all_n = set([str(i) for i in range(10)])
rest = all_n-D
rest_list = list(rest)
rest_list.sort()
digit = len(str(N))
ans = 10**8
# print(rest_list)
for candidate in itertools.product(rest_list, repeat=digit):
    tmp = "".join(candidate)
    # print(tmp)
    if int(tmp) >= N:
        ans = min(ans, int(tmp))
if ans != 10**8:
    ...
    s = rest_list[0] if rest_list[0] > "0" else rest_list[1]
    print(s+rest_list[0]*digit)
