...
def main():
    ...
    for i in range(double//2):
        # print(sortedA[i], sortedA[-i-1])
        tans += 2*(sortedA[i]*sortedA[double-i-1])
    # for i in range(N-single+1):
    #     fixed = sortedA[i:i+single]
    #     yet = sortedA[:i]+sortedA[i+single:]
    #     plus = [x+y for x, y in zip(yet[:double], yet[double:][::-1])]
    # # print(fixed)
    # # print(plus)
    #     tmp = 0
    #     for i in fixed:
