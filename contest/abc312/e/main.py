...
def main():
    ...
    for c in range(N):
        x = LI()
        for i in range(x[0]+1, x[3]+1):
            for j in range(x[1]+1, x[4]+1):
                for k in range(x[2]+1, x[5]+1):
                    cubes[i*101*101+j*101+k] = c
    comb = [set() for i in range(N)]
    for i in range(101):
        for j in range(101):
            for k in range(101):
                for l in [1, 101, 101*101]:
                    if cubes[i*101*101+j*101+k] != -1 and cubes[i*101*101+j*101+k+l] != -1 and cubes[i*101*101+j*101+k] != cubes[i*101*101+j*101+k+l]:
                        # print(i, j, k, l, cubes[i*101*101+j*101+k], cubes[i*101*101+j*101+k+l])
                        comb[cubes[i*101*101+j*101+k]].add(cubes[i*101*101+j*101+k+l])
                        comb[cubes[i*101*101+j*101+k+l]].add(cubes[i*101*101+j*101+k])
    print(*[len(x) for x in comb], sep="\n")
