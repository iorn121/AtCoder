...
for h in range(8):
    for w in range(8):
        if G[h][w] == "Q":
            row.add(h)
            col.add(w)
row = set([i for i in range(8)])-row
col = set([i for i in range(8)])-col
if not len(row) == len(col) == 5:
    exit(print("No Answer"))
col_perm = itertools.permutations(list(col))
row = list(row)
for col in col_perm:
    nG = [list(G[i][:]) for i in range(8)]
    # print(nG)
    for h, w in zip(row, col):
        nG[h][w] = 'Q'
    if check(nG, 8):
        for i in range(8):
            print(*nG[i], sep="")
