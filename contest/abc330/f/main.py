...
def main():
    ...
    def judge(edge,x,y):
        cnt=0
        for px,py in points:
            if px<x or x+edge<px:
                cnt+=min(abs(px-x),abs(px-(x+edge)))
            if py<y or y+edge<py:
                cnt+=min(abs(py-y),abs(py-(y+edge)))
        # print(edge,x,y,cnt)
        return cnt<=K
    def solve():
        ok=-1
        ...
        while ng-ok>1:
            mid=(ok+ng)//2
            sx=floor((sumx-mid*N//2)//N)
            sy=floor((sumy-mid*N//2)//N)
            if judge(mid,sx,sy) or judge(mid,sx+1,sy) or judge(mid,sx,sy+1) or judge(mid,sx+1,sy+1):
                ng=mid
            else:
                ok=mid
        return ng
