        ...
        for i in range(judge+1):
            if P[i] == judge:
                ...
                if P[i] != skip:
                    ...
                    tmp2.append(P[i])
        if left+1 == right:
            ans.append([right+1, judge-1])
            P = tmp1+[judge-1, judge]
            # print(*P)
            P.pop()
            P.pop()
            judge -= 2
            ...
            ans.append([right+1, judge-1])
            ans.append([judge-1, judge-1])
            x = tmp1.pop()
            P = tmp1+[judge-1, x, judge]
            # print(*P)
            P.pop()
            judge -= 1
        elif right < left:
            ans.append([right+1, left-1])
            ans.append([left-1, judge-1])
            P = tmp2+[skip]+[judge-1, judge]
            # print(*P)
            P.pop()
            P.pop()
            judge -= 2
