...
def main():
    ...
    for s,t in zip(S,T):
        if s=='A' and t=='B':
            ...
            lastBtoB=max(lastBtoB,i)
        elif s=='A' and t=='A':
            firstAtoA=min(firstAtoA,i)
        i+=1
    # print(AtoB)
    # print(BtoA)
    ans=0
    for i in range(len(BtoA)-1,-1,-1):
        if len(AtoB)>0 and BtoA[i]<AtoB[-1]:
            ans+=1
            x=AtoB.pop()
            lastBtoB=max(lastBtoB,x)
            firstAtoA=min(firstAtoA,BtoA[i])
        elif BtoA[i]<lastBtoB:
            ans+=1
            firstAtoA=min(firstAtoA,BtoA[i])
        else:
            print(-1)
