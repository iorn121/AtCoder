...
def main():
    ...
    for taka in range(1, 10):
        for aoki in range(1, 10):
            if taka == aoki:
                if Sd[taka]+Td[taka] > K-2:
                    continue
                ta = 0
                ao = 0
                for sx in range(1, 10):
                    ta += sx*10**(Sd[sx]+1 if sx == taka else Sd[sx])
                for tx in range(1, 10):
                    ao += tx*10**(Td[tx]+1 if tx == aoki else Td[tx])
                if ta > ao:
                    win += (K-Sd[taka]-Td[taka])*(K-Sd[taka]-Td[taka]-1)
            else:
                if Sd[taka]+Td[taka]+1 > K:
                    continue
                if Sd[aoki]+Td[aoki]+1 > K:
                    continue
                ta = 0
                ao = 0
                for sx in range(1, 10):
                ...
                for tx in range(1, 10):
                    ao += tx*10**(Td[tx]+1 if tx == aoki else Td[tx])
                if ta > ao:
                    win += (K-Sd[taka]-Td[taka])*(K-Sd[aoki]-Td[aoki])
