...
plus_n = len(plus)
minus_n = len(minus)
plus_scores = [0]*(plus_n*50+1)
minus_scores = [0]*(minus_n*50+1)
plus_scores[0] = 1
minus_scores[0] = 1

for i in range(plus_n):
    tmp = plus_scores[:]
    for j in range(plus_n*50):
        if plus_scores[j] > 0 and plus[i]+j <= plus_n*50:
            tmp[plus[i]+j] += plus_scores[j]
    plus_scores = tmp[:]

for i in range(minus_n):
    tmp = minus_scores[:]
    for j in range(minus_n*50):
        if minus_scores[j] > 0 and minus[i]+j <= minus_n*50:
            tmp[minus[i]+j] += minus_scores[j]
    minus_scores = tmp[:]
...
for p, m in zip(plus_scores[1:], minus_scores[1:]):
    ans += p*m*pow(2, just)
ans += pow(2, just)-1
print(ans)
