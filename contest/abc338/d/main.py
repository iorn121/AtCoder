...
class LazySegmentTree:
    ...
    def gindex(self, left, right, k=1, l=0, r=None):
        if r is None:
            r = self.size
        ...
        mid = (l + r) // 2
        self.propagate(k)
        il = self.gindex(left, right, k * 2, l, mid)
        ir = self.gindex(left, right, k * 2 + 1, mid, r)
        if il is not None:
            return il
        return ir
    ...
    def update(self, left, right, x):
        g = self.gindex(left, right)
        self.lazy[g] += (right - left) * x
        self.propagate(g)
    ...
    def query(self, left, right):
        self.gindex(left, right)
        return min(self.data[left:right])


def main():
    N,M = MI()
    X=LI()
    lst=LazySegmentTree(N,op=min,e=0)
    ans=0
    for fr,to in zip(X,X[1:]):
        ...
        if plus==minus:
            ...
            ans+=plus
            lst.add(fr-1,to-1,plus-minus)
        else:
            ans+=minus
            lst.add(to-1,N-1,minus-plus)
            if fr>1:
                lst.add(0,fr-1,minus-plus)
        print(lst.data[:N])
    min_cost=lst.query(0,N)
    print(ans,min_cost)
    print(ans+min_cost)
