...
def main():
    H,W=MI()
    field=SS(H)
    row_sum=[[0]*W for _ in range(H)]
    col_sum=[[0]*W for _ in range(H)]
    for i in range(H):
        for j in range(W):
            if field[i][j]==".":
                if j==0:
        ...
        for j in range(W-1,-1,-1):
            if field[i][j]=="." and j!=W-1:
                row_sum[i][j]=max(row_sum[i][j],row_sum[i][j+1])
    # print(row_sum)
    for j in range(W):
        for i in range(H):
            if field[i][j]==".":
        ...
        for i in range(H-1,-1,-1):
            if field[i][j]=="." and i!=H-1:
                col_sum[i][j]=max(col_sum[i][j],col_sum[i+1][j])
    # print(col_sum)
    ans=0
    for i in range(H):
        for j in range(W):
            ans=max(ans,row_sum[i][j]+col_sum[i][j]-1)
    print(ans)
