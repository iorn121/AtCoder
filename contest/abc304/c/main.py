...
def main():
    ...
    # N人のX,Y座標を取得して、距離を計算（平方根は2乗したまま）
    for i in range(N):
        x,y=MI()
        X[i]=x
        Y[i]=y
        for j in range(i):
            if (X[j]-X[i])**2+(Y[j]-Y[i])**2<=D**2:
                distance[i].append(j)
                distance[j].append(i)
    # print(distance)
    # 人0からの距離がD以下の人を列挙し、一度確認した人を二度カウントしないようにsetで管理する
    q=collections.deque()
    q.append(0)
    checked=set([0])
    while q:
        now=q.popleft()
        for nxt in distance[now]:
            if nxt not in checked:
                q.append(nxt)
                checked.add(nxt)
    for i in range(N):
        print("Yes" if i in checked else "No")
