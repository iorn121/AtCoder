...
def main():
    ...
    for i in range(N):
        if X in exist:
            offset_n = exist[X]
            loop_n = i-exist[X]
            loop_sum = ruiseki[-1]-ruiseki[exist[X]]
            offset_sum = ruiseki[exist[X]]
            # print("last i:", i)
            break
        exist[X] = i
        ruiseki.append(ruiseki[-1]+X)
        X = (X**2) % M
    # print(exist)
    ...
    if offset_n == -1:
        ...
        # print(offset_n, loop_n, loop_sum, offset_sum)
        N -= offset_n
        m = N//loop_n
        n = N % loop_n
        ans = offset_sum+m*loop_sum+ruiseki[n+offset_n]-ruiseki[offset_n]
    print(ans)
