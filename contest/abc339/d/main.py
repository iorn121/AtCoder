...
def main():
    ...
    #         for i,j in dxy:
    #             nx,ny=x+i,y+j
    #             if 0<=nx<N and 0<=ny<N and root[nx][ny]==-1 and field[nx][ny]!="#":
    #                 root[nx][ny]=step+1
    #                 q.append((nx,ny,step+1))
    #     # print(root)
    #     if root[p2x][p2y]==-1:
    #         exit(print(-1))
    #     return root,root[p2x][p2y]
    
    # def proceed(p2x,p2y,command):
    #     cnt=[]
    #     for c in command:
    #         cx,cy=p2x,p2y
    #         if c=="U":
    ...
    #         else:
    #             cy+=1
    #         if 0<=cx<N and 0<=cy<N and field[cx][cy]!="#":
    #             p2x,p2y=cx,cy
    #         else:
    #             cnt.append(c)
    #     return p2x,p2y,cnt
    ...
    # # ルートを逆算
    # def rec(p2x,p2y,root,direction):
    #     command=[]
    #     sx,sy=p2x,p2y
    #     while root[sx][sy]!=0:
    #         for i,j in direction:
    #             nx,ny=sx+i,sy+j
    #             if 0<=nx<N and 0<=ny<N and root[nx][ny]==root[sx][sy]-1:
    #                 if i==1:
    #                     command.append("U")
    #                 elif i==-1:
    #                     command.append("D")
    ...
    #     tplayers=[p[:] for p in players]
    #     tplayers[0]=players[1][:]
    #     root,num=bfs(players)
    #     tmp=0
    #     for direction in itertools.permutations(dxy):
    #         command=rec(players[1][0],players[1][1],root,direction)
    #         p2x,p2y,cnt=proceed(players[1][0],players[1][1],command)
    #         print(cnt)
    #         cn=0
    #         for c in cnt:
    #             if c in command:
    ...
    #     print(tmp)
    #     ans+=num-tmp
    #     players=[p[:] for p in tplayers]
    # print(ans)

    dp=[[INF]*N**2 for i in range(N**2)]
    dp[players[0][0]*N+players[0][1]][players[1][0]*N+players[1][1]]=0
    q=collections.deque()
    q.append((players[0][0],players[0][1],players[1][0],players[1][1]))
    while q:
        p1x,p1y,p2x,p2y=q.popleft()
        if p1x==p2x and p1y==p2y:
            break
        for i,j in dxy:
            np1x,np1y=p1x+i,p1y+j
            np2x,np2y=p2x+i,p2y+j
            if not(0<=np1x<N and 0<=np1y<N and field[np1x][np1y]!="#"):
                np1x,np1y=p1x,p1y
            if not(0<=np2x<N and 0<=np2y<N and field[np2x][np2y]!="#"):
                np2x,np2y=p2x,p2y
            if dp[np1x*N+np1y][np2x*N+np2y]==INF:
                dp[np1x*N+np1y][np2x*N+np2y]=min(dp[np1x*N+np1y][np2x*N+np2y],dp[p1x*N+p1y][p2x*N+p2y]+1)
                q.append((np1x,np1y,np2x,np2y))
    ans=min(dp[i][i] for i in range(N**2))
