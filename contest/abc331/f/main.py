...
def main():
    ...
    B = 5
    mod = [998244353, 1000000007, 1000000009, 1000000021, 1000000033]
    base = [random.randint(0, mod[i]) for i in range(B)]

    class Hash:
        def __init__(self, h1=0, h2=0, pw=1):
            self.h1 = h1
            self.h2 = h2
            self.pw = pw

    def op(lhs, rhs):
        res = [Hash() for _ in range(B)]
        for i in range(B):
    ...
    def e():
        return [Hash() for _ in range(B)]

    def gen(c):
        res = [Hash() for _ in range(B)]
        for i in range(B):
            res[i].h1 = res[i].h2 = ord(c)
            res[i].pw = base[i]
        return res
    ...
    init = [gen(S[i]) for i in range(N)]
    seg = SegTree(init, op, e)
    ...
    for _ in range(Q):
        ...
        if cmd == 1:
            ...
            l -= 1
            h = seg.prod(l, r)
            flag = all(h[i].h1 == h[i].h2 for i in range(B))
