...
for total in range(1, F+1):
    for sugar in range(0, total+1):
        if total >= 100*A and dp[total-100*A][sugar]:
            dp[total][sugar] = True
        if total >= 100*B and dp[total-100*B][sugar]:
            dp[total][sugar] = True
        if sugar >= C and dp[total-C][sugar-C]:
            dp[total][sugar] = True
        if sugar >= D and dp[total-D][sugar-D]:
            dp[total][sugar] = True

        if dp[total][sugar] and (100+E)*sugar <= E*total and ans1*sugar >= ans2*total:
            ans1 = total
            ans2 = sugar
print(ans1, ans2)
