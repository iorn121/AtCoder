...
int main() {
  ...
  if(sum%3!=0) ans="No";
  sum/=3;
  if(sum!=c[0][0]+c[1][1]+c[2][2]) ans="No";
  REP(i,2) {
    if(c[i][0]-c[i+1][0]!=c[i][1]-c[i+1][1]||c[i][1]-c[i+1][1]!=c[i][2]-c[i+1][2]) ans="No";
    if(c[0][i]-c[0][i+1]!=c[1][i]-c[1][i+1]||c[2][i]-c[2][i+1]!=c[1][i]-c[1][i+1]) ans="No";
  }
  cout << ans << endl;
  return 0;
}
